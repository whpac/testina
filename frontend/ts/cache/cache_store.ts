export default class CacheStore {
    ...
    public async GetResource(request: Request): Promise<Response | undefined> {
        ...
        if(cached === undefined) {
            console.warn('Zasób nie istnieje w pamięci podręcznej');
            return undefined;
        }
        ...
        if(new Date(expire_date) < new Date()) {
            console.warn('Zasób jest przeterminowany');
            return undefined;
        }

        //console.info('Wczytywanie zasobu ' + request.url);
        return cached;
    }
    ...
    public async SaveResponse(request: Request, response: Response) {
        ...
        //console.info('Zapisywanie zasobu ' + request.url);
